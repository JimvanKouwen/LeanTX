...
extern uint8_t telemetryStreaming;  // >0 (true) == data is streaming in. 0 = no
                                    // data detected for some time
...
int setTelemetryValue(TelemetryProtocol protocol, uint16_t id, uint8_t subId,
                      ...
                      uint32_t prec);
int setTelemetryText(TelemetryProtocol protocol, uint16_t id, uint8_t subId,
                     uint8_t instance, const char* text);
...
class OutputTelemetryBuffer
{
 public:
  ...
  bool isAvailable() { return destination == TELEMETRY_ENDPOINT_NONE; }

  void pushByte(uint8_t byte)
  {
    if (size < TELEMETRY_OUTPUT_BUFFER_SIZE) data[size++] = byte;
  }
