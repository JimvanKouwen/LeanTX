...
class OutputLineButton : public ListLineButton
{
 ...
 public:
  ...
  void refresh() override
  {
    ...
    char s[32];
    formatConfigValue(s, sizeof(s), output->min, PREC1,
                         nullptr, -LIMITS_MIN_MAX_OFFSET, true);
    lv_label_set_text(min, s);

    formatConfigValue(s, sizeof(s), output->max, PREC1,
                         nullptr, +LIMITS_MIN_MAX_OFFSET, true);
    lv_label_set_text(max, s);

    formatConfigValue(s, sizeof(s), output->offset, PREC1, nullptr, 0, true);
