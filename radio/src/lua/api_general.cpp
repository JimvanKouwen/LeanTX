...
Queues a raw CRSF frame of any type (0..255). Trusted scripts are responsible
for frame semantics, including channel and command frames. Payload bytes must
be exact integers in 0..255 and the complete frame must fit the output buffer.
COMMAND_ID includes both CRC8_BA and CRC8_D5.
...
{
  if (!LuaHostApi::rawCrsfActive()) {
    lua_pushnil(L);
    return 1;
  }
  if (lua_gettop(L) == 0) {
    lua_pushboolean(L, LuaHostApi::rawCrsfAvailable());
    return 1;
  }
  if (!LuaHostApi::rawCrsfAvailable()) {
    lua_pushboolean(L, false);
    return 1;
  }
  int isCommandInteger = 0;
  auto command = lua_tointegerx(L, 1, &isCommandInteger);
  ...
  if (lua_type(L, 1) != LUA_TNUMBER || !isCommandInteger ||
      command < 0 || command > 255 || lua_tonumber(L, 1) != command ||
      length > LuaHostApi::rawCrsfPayloadCapacity(command)) {
    lua_pushboolean(L, false);
    return 1;
  }
  uint8_t payload[LuaHostApi::RawCrsfMaxPayload];
  for (size_t i = 0; i < length; ++i) {
    lua_rawgeti(L, 2, i + 1);
    int isInteger = 0;
    auto byte = lua_tointegerx(L, -1, &isInteger);
    bool valid = lua_type(L, -1) == LUA_TNUMBER && isInteger && byte >= 0 && byte <= 255 && lua_tonumber(L, -1) == byte;
    lua_pop(L, 1);
    if (!valid) {
      lua_pushboolean(L, false);
      return 1;
  ...
  lua_pushboolean(L, LuaHostApi::rawCrsfSend(command, payload, length));
  return 1;
}
#endif
...
static int luaSetTelemetryValue(lua_State * L)
{
  ...
  lua_pushboolean(L, LuaHostApi::publishTelemetry(id, subId, instance, value,
                                                 unit, prec, name ? name : name_buf));
  return 1;
}
...
{
  int port_nr = LuaHostApi::luaSerialPort();
  if (port_nr < 0) return 0;

  uint32_t baudrate = luaL_checkunsigned(L, 1);
  LuaHostApi::setSerialBaudrate(port_nr, baudrate);
...
static int luaSerialGetPower(lua_State* L)
{
  ...
  bool enabled;
  if (LuaHostApi::serialPower(port_nr, enabled)) {
...
static int luaSerialSetPower(lua_State* L)
{
  ...
  lua_pushboolean(L, LuaHostApi::setSerialPower(port_nr, value));
  return 1;
}
#endif
...
static int luaSetIMU_X(lua_State* const L)
{
  ...
  LuaHostApi::setIMU_X(offset, range);
#endif
  lua_pushboolean(L, true);
  return 1;
}
...
static int luaSetIMU_Y(lua_State* const L)
{
  ...
  LuaHostApi::setIMU_Y(offset, range);
#endif
  lua_pushboolean(L, true);
  return 1;
}
